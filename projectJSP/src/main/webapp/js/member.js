...
function checkId(){
	...
	if(id == ''){
		document.getElementById("idDiv").innerHTML = "먼저 아이디를 입력";
	}
	else{
		window.open("./checkId.jsp?id="+id,"mywindow","width = 400 height = 300 top = 300 left = 700");
	}
}
...
function memberWrite() {
	...
	let checkid = document.getElementById("check").value;
	...
	if(!id){
		...
		//id중복체크 후 id를 바꾸면 다시 중복체크해야함
		if (id != checkid) {
			document.getElementById("idDiv").innerHTML = "아이디 중복 체크를 해주세요.";
			return false;
		}
	}
	if(!pwd){
